...
function getEvent(event) {
  return `<div class="card border-secondary mb-3" style="width: 18rem; display: flex; flex-direction: column;">
  <div style="overflow: hidden; height: 12rem;">
      <img src="${event.image}" class="card-img-top" alt="${event.name}" style="width: 100%; object-fit: cover; height: 100%;">
  </div>
  <div class="card-body" style="display: flex; flex-direction: column; justify-content: space-between;">
      ...
      <div style="align-self: flex-end;">Price: $${event.price}</div>
  </div>
</div>
...
function createCheckbox(cat, i) {
  return `
    <div class="form-check form-check-inline m-0">
        ...
        <label class="form-check-label" for="category${i}">${cat}</label>
    </div>`
}
...
function filterCheckbox(events, checkbox) {
  ...
  if(checkbox.length == 0){
    ...
    <div>
    <h3 class="display-1">There are no matches for your search</h3>
    </div>`
    return
}
  if (checkbox.length > 0) {
